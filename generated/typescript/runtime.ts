...
export function loadFrom(lookup: (key: string) => string | undefined): SidecarEnvValues {
  return {
    bind: nonEmpty(lookup("PRAXONNE_SIDECAR_BIND")) ?? "127.0.0.1:9090",
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const bind = pick(["PRAXONNE_SIDECAR_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "127.0.0.1:9090");
  if (bind !== undefined) out["PRAXONNE_SIDECAR_BIND"] = bind;
  return out;
}
