...
#   - livenessProbe exec ["/praxonne-sidecar", "probe"]
#   - no readinessProbe
...
# Binary lives at /<bin> so kubelet exec ["/praxonne-sidecar", "probe"] matches the
# ores-otel sidecar contract (no curl, no /usr/local/bin prefix required).
COPY --from=build --chown=65532:65532 "/usr/local/bin/praxonne-sidecar" "/praxonne-sidecar"
ENV PRAXONNE_SIDECAR_BIND=127.0.0.1:9090 \
    ORES_OTEL_SIDECAR_BIND=127.0.0.1:9090 \
    OTEL_SERVICE_NAME=praxonne-sidecar
USER 65532:65532
ENTRYPOINT ["/praxonne-sidecar"]
